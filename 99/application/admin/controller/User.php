...
class User extends Base
{
    ...
    public function cash()
    {
        ...
        $balance = Db::name('balance')->alias('b')->field('b.*,u.username,u.nickname,u.oid,u.managername,bc.accntnm,bc.content,bc.accntno,bi.address,bi.qrcode')
            ->join('__USERINFO__ u','u.uid=b.uid')->join('__BANKINFO__ bi','u.uid=bi.uid and bi.id=b.bankid','left')->join('__BANKCARD__ bc','u.uid=bc.uid and bc.id=b.bankid','left')
            ->where($where)->order('bpid desc')->paginate($pagenum,false,['query'=>$getdata]);
        $where['b.isverified'] = 1;
...
//				->find();
    ...
    public function addprice()
    {
        ...
        if($post['bpprice']>0) {
            ...
            $remark = $post['remark']?$post['remark']:'Successfully added funds to the system';
            $bptype = 2;
        }else{
            ...
            $remark = $post['remark']?$post['remark']:'System deduction successful';
            $bptype = 6;
        }
