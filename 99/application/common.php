...
function amount_name(){
    $amount_name = db('config')->field('value')->where(['name'=>'amount_name'])->find();
    return $amount_name['value'];
}

function amount_name_admin(){
    $amount_name = db('config')->field('value')->where(['name'=>'amount_name_admin'])->find();
    return $amount_name['value'];
}
...
function GetUserOidInfo($uid,$field)
{
	...
	//通过邀请码的uid查询所属员工信息
	$res['yuangong'] = Db::name('userinfo')->field($field)->where('uid',$useroid)->find();
...
function getconf($field='')
{
    ...
    if(!$conf_cache){
        ...
        foreach ($conf as $k => $v) {
            ...
            $conf_value[$v['name'].'_desc'] = $v['title'];
            $conf_value[$v['name'].'_extra'] = $v['extra'];
        }
