...
class User extends Base
{
	...
	public function aotucard()
    {
        if(input('post.')){
            ...
            if($data['real_type'] == 1){
                $cardinfo = Db::name('cardinfo')->where(['uid'=>$data['uid']])->find();
                if($cardinfo){
                    $data['ctime'] = time();
                    $data['is_check'] = 1;
                    unset($data['real_type']);
                    if (Db::name('cardinfo')->where(['uid'=>$data['uid']])->update($data)) {
                        return json(['code'=>200,'data'=>[],'msg'=>'success']);
                    }else{
                        return json(['code'=>-66,'data'=>[],'msg'=>'error']);
                    }
                }
                $data['ctime'] = time();
                $data['is_check'] = 1;
                unset($data['real_type']);
                if (Db::name('cardinfo')->insertGetId($data)) {
                    return json(['code'=>200,'data'=>[],'msg'=>'success']);
                }else{
                    return json(['code'=>-66,'data'=>[],'msg'=>'error']);
                }
            }else if($data['real_type'] == 2){
                if(!$data['front_pic']){
                    return json(['code'=>-67,'data'=>[],'msg'=>'请上传证件正面照片']);
                }
                if(!$data['reverse_pic']){
                    return json(['code'=>-68,'data'=>[],'msg'=>'请上传证件背面照片']);
                }
                $data['utime'] = time();
                $data['up_check'] = 1;
                unset($data['real_type']);
    ...
    public function cardinfo()
    {
        ...
        $cardinfo = Db::name('cardinfo')->where(['uid'=>$uid])->find();
        return json(['code'=>200,'data'=>$cardinfo,'msg'=>'success']);
    }
    ...
    public function ajax_price_list()
    {
   	...
   	public function addbalance()
   	{
   		...
   		if($post['pay_type']=='mcb_alipay'||$post['pay_type']=='mcb_wxpay'||$post['pay_type']=='mcb_qqpay'||$post['pay_type']=='mcb_bankpay'){
			echo json_encode(array('data'=>'已提交充值信息!','payno'=>$data['balance_sn']));
			exit;
		}
		//秒冲宝
