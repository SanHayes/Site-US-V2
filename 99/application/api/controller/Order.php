	...
	public function addorder()
	{
		...
		*/
		if($data['order_price'] > $conf['order_max_price']){
			return json(['code'=>-52,'data'=>[],'msg'=>'单笔持仓最大为'.$conf['order_max_price'].'！']);
		}
		$allcount = Db::name('order')->where(array('ostaus'=>0,'uid'=>$data['uid']))->count();
		if($allcount >  $conf['max_order_count']){
			return json(['code'=>-53,'data'=>[],'msg'=>'最大持仓数量为'.$conf['max_order_count'].'！']);
		}
		//验证是否开市
		...
		//验证用户是否被冻结
		if($user['ustatus'] == 1){
			return json(['code'=>-54,'data'=>[],'msg'=>'您的账户已被冻结,请联系在线客服！']);
		}
		
		if($user['ustatus'] == 2){
			return json(['code'=>-55,'data'=>[],'msg'=>'您的账户已被限制交易,请联系在线客服！']);
		}

		if($conf['sys_buy_once'] == 1) {
			$iorder = Db::name('order')->where('ostaus',0)->where('uid',$data['uid'])->find();
			return json(['code'=>-56,'data'=>[],'msg'=>'您有待结算的持仓，请勿重复下单！']);
		}

		//手续费
		$web_poundage = sprintf("%.7f",round($data['order_price']*$conf['web_poundage']/100,2));
		
		//验证余额是否够
		if($user['usermoney'] < $data['order_price'] + $web_poundage){
			return json(['code'=>-57,'data'=>[],'msg'=>'您得余额不足，请充值！']);
		}
...
// 		}
		...
		//$adddata['buyprice'] = $data['newprice'] - $web_poundage;
		$adddata['point'] = $point;
		...
		try{
			...
			if($ids){
				...
				if($editmoney){
					...
					// 回滚事务
					Db::rollback();
					return json(['code'=>-61,'data'=>[],'msg'=>'下单失败，请重试！']);
				}
			...
			// 回滚事务
			Db::rollback();
			return json(['code'=>-62,'data'=>[],'msg'=>'操作失败！']);
		}
	}
	...
	function getResult() {
		...
		$db_order = db('order');
		$order = $db_order->field('oid,uid,pid,ostaus,ploss,sellprice')->where(['oid'=>$oid])->find();
		if(!$order){
			return json(['code'=>-63,'data'=>[],'msg'=>'订单不存在']);
		}
		return json(['code'=>200,'data'=>$order,'msg'=>'']);
	}
    ...
    public function getSecondsList() {
        $uid = input('get.uid');
        $ostaus = input('get.status');
		//持仓信息
		$map = array('uid'=>$uid,'ostaus'=>$ostaus);
    
		$hold = Db::name('order')->where($map)->order('oid desc')->paginate(input('get.limit'),false,['type' => 'Bootstrap','var_page' => 'page','page' => input('get.page')]);
        $hold->getCollection()->each(function ($item, $key) use ($hold) {
            $item['pro'] = GetProData($item['pid'],'pi.*');
            $item['buytime_date'] = date('Y-m-d H:i:s', $item['buytime']);
            $item['selltime_date'] = date('Y-m-d H:i:s', $item['selltime']);
            $item['time'] = time();
		...
		if($hold){
		    return json(['code'=>200,'data'=>$hold,'msg'=>'']);
		}else{
			return json(['code'=>-64,'data'=>[],'msg'=>'无可用数据']);
		}		
    }
	/**
	 * ajax 获取用户未平仓订单，
	...
	{
		$uid = input('get.uid');
		$hold = Db::name('order')->where(array('uid'=>$uid, 'ostaus'=>1, 'selltime' => ['lt', time()]))->order('oid desc')->paginate(input('get.limit'),false,['type' => 'Bootstrap','var_page' => 'page','page' => input('get.page')]);
        $hold->getCollection()->each(function ($item, $key) use ($hold) {
            $item['buytime'] = date('Y-m-d H:i:s', $item['buytime']);
            $item['selltime'] = date('Y-m-d H:i:s', $item['selltime']);
            $item['pro'] = GetProData($item['pid'],'pi.*');
