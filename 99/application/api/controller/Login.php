    ...
    */
    public function login()
    {
        if (input('post.')) {
            $ip = $this->ip;
            $resip = array();
            $resip = db('blacklist')->where('ip', $ip)->find();
            if(!empty($resip) && $resip['id'] > 0){
                return json(['code'=>-39,'data'=>[],'msg'=>'您的IP被禁止登陆，请联系客服']);
            }
            
            $data = input('post.');

            if (!isset($data['username']) || !isset($data['pwd'])) {
                return json(['code'=>-40,'data'=>[],'msg'=>'信息为空']);
            }
            ...
            if (!$result) {
                return json(['code'=>-41,'data'=>[],'msg'=>'登录失败,用户名不存在']);
            }
            //只有代理商和普通客户可以前端登录
            if (!in_array($result['otype'], array(0, 101))) {
                return json(['code'=>-42,'data'=>[],'msg'=>'您无权登录']);
            }

            //帐号冻结
            if ($result['ustatus'] == 1) {
                return json(['code'=>-43,'data'=>[],'msg'=>'登录失败,您的账户暂时被冻结']);
            }

            if ($result['upwd'] != $data['pwd']) {
                return json(['code'=>-44,'data'=>[],'msg'=>'登录失败,密码错误']);
            }
            ...
            $user = db('userinfo')->where('username', $data['username'])->field("uid,username,utime,usermoney,nickname,lastlog")->find();
            $user['token'] = md5($user['uid'].$user['username']);
            return json(['code'=>200,'data'=>$user,'msg'=>'']);
        }
    }
    ...
    public function register()
    {
        if(input('post.')){
            ...
            if(!isset($data['username']) || empty($data['username'])){
                return json(['code'=>-45,'data'=>[],'msg'=>'请输入用户名']);
            }else{
                if (strpos($data['username'], '+') !== false) {
                    $data['username'] = str_replace("+","",$data['username']);
                }
            }
            $iscode = db('config')->field('value')->where(['name'=>'sys_invit','status'=>1])->find();
            if($iscode['value'] == 1){
                if(!isset($data['code']) || empty($data['code'])){
                    return json(['code'=>-66,'data'=>[],'msg'=>'请输入验证码']);
                }else{
                    $user_code = Db::name('usercode')->where(['number'=>$data['username']])->find();
                    if($user_code['code'] != $data['code']){
                        return json(['code'=>-67,'data'=>[],'msg'=>'验证码不正确']);
                    }
                    if(time() - $user_code['time']  > 600){
                        return json(['code'=>-69,'data'=>null,'msg'=>'验证码已超时']);
                    }
                }
            }
            
            if(!isset($data['upwd']) || empty($data['upwd'])){
                return json(['code'=>-46,'data'=>[],'msg'=>'请输入密码']);
            }
            if(!isset($data['upwd2']) || empty($data['upwd2'])){
                return json(['code'=>-47,'data'=>[],'msg'=>'请再次输入密码']);
            }
            if($data['upwd'] != $data['upwd2']){
                return json(['code'=>-48,'data'=>[],'msg'=>'两次输入密码不同']);
            }
            if($data['oid']){
                //判断邀请码是否存在
                $codeid = checkcode($data['oid']);
                if(!$codeid){
                    return json(['code'=>-49,'data'=>[],'msg'=>'此邀请码不存在']);
                }
            }
            
            unset($data['phonecode'],$data['code'],$data['upwd2'],$data['agenttype'],$data['otype']);
            if(check_user('username',$data['username'])){
                return json(['code'=>-50,'data'=>[],'msg'=>'该用户名已存在']);
            }
            $data['utime'] = $data['logintime'] = $data['lastlog'] = time();
            $data['upwd'] = $data['upwd'];
            $data['epwd'] = $data['epwd']?$data['epwd']:$data['upwd'];
            $data['nickname'] = trim($data['nickname'])?$data['nickname']:trim($data['username']);
            $data['icard'] = trim($data['icard']);
            $data['scard'] = trim($data['icard']);
            ...
            if(isset($_SESSION['fid']) && $_SESSION['fid']>0){
                $fid = $_SESSION['fid'];
                $fid_info = Db::name('userinfo')->where(array('uid'=>$fid,'otype'=>101))->value('uid');
                if($fid_info){
                    $data['oid'] = $fid;
                }
            }
            $data['managername'] = '';//Db::name('userinfo')->where(array('uid'=>$data['oid'],'otype'=>101))->value('username');
            ...
            if ($uid) {
                ...
                return json(['code'=>-51,'data'=>[],'msg'=>'注册失败']);
            }
    ...
    public function logout()
    {
        ...
        db('userinfo')->where(['uid'=>$uid])->update(['online'=>0,'update_time'=>time()]);
