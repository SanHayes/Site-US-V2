...
class Goods extends Base
{
	...
	public function goods()
	{
		...
		$uid = input('param.uid');
		if(!$pid){
			return json(['code'=>-1,'data'=>[],'msg'=>'商品ID无效']);
		}
		$user = Db::name('userinfo')->field('ustatus,usermoney,uid,oid,username')->where('uid',$uid)->find();
		//获取产品实时行情
		...
		//时间最小下注金额
		$promoney = $pro['promoney'];
		if($promoney){
			$promoney = explode(',',$promoney);
		}
		//点位间隔
		...
		$risk = db('risk')->find();
		$userbind = db('userbind')->where(['uid'=>$uid,'pid'=>$pid])->find();
		$data = [];
		//$data['pro'] = $pro;
		$data['protime'] = $protime;
		$data['promoney'] = $promoney;
		//$data['propoint'] = $propoint;
		//$data['proscale'] = $proscale;
		$data['pay_choose_arr'] = $pay_choose_arr;
		$data['userbind'] = $userbind ? 1 : 0;
		$data['isopen'] = $isopen;
		$data['web_poundage'] = getconf('web_poundage');
		$data['userinfo'] = $user;
		$data['risk'] = $risk;
