...
if (is_file($lockFile)) {
    die("<script>window.location.href = '/99/admin/login/login'</script>");
}
if ($_GET['c'] = 'start' && isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mysqlDatabase = isset($_POST['database']) ? $_POST['database'] : 'site';
    $mysqlPreFix = 'wp_';
    //$mysqlPreFix = isset($_POST['prefix']) ? $_POST['prefix'] : 'wp_';
    ...
    //检测能否读取安装文件
    $sql = @file_get_contents(WWW_ROOT . 'site.sql');
    if (!$sql) {
        die("<script>alert('请检查site.sql是否有读取权限！');</script>");
    }
    //替换表前缀
    //$sql = str_replace("`wp_", "`{$mysqlPreFix}", $sql);
    //链接数据库
    ...
    $result = $link->query("UPDATE {$mysqlPreFix}userinfo SET `username` = '{$adminUserName}',`upwd` = '{$adminPassword}',`utel` = '{$email}' WHERE `username` = 'admin'");
    if (!$result) {
        die("<script>alert('安装失败！:$error');history.go(-1)</script>");
    }
    $databaseConfig = include "../99/application" . DS . "database_default.php";
    //替换数据库相关配置
    ...
    $putConfig = @file_put_contents("../99/application" . DS . "database.php", "<?php\nreturn \n" . var_export($databaseConfig, true) . "\n;");
    if (!$putConfig) {
...
<head>
    ...
    <style type="text/css">
        ...
        body {
            height: 100%;
            background-image: url("img/installbg.jpg");
...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-4 col-md-offset-4" style="margin-top: 20px;background-color: rgba(255,255,255,.5);padding: 30px;border-radius: 5px">
                <div id="cms-box">
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="hostname" class="form-control" placeholder="请输入主机地址、端口号可选" value="127.0.0.1">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">数据库名</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="database" class="form-control" placeholder="请输入数据库名" value="site">
                                    </div>
                                </div>
                                <div class="form-group">
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="email" class="form-control" placeholder="请输入管理员邮箱" value="admin@admin.com">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">密码</label>
